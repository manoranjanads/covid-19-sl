  ...
  $.extend(Plugin.prototype, {
    ...
    createMarquee: function() {
      ...
      for (var i = 0; i < spawnAmount; i++) {
        ...
        plugin.marqueeSpawned[i].currentPos =
          widthToIgnore + contentWidth * i + config.padding * i; //initial positioning
        ...
        plugin.marqueeSpawned[i].el.css(
          "left",
          plugin.marqueeSpawned[i].currentPos + config.padding + "px"
        ); //setting left according to postition
      ...
      if (marqueed_el.hovered == false) {
        if (marqueed_el.counter > 0) {
          ...
          marqueed_el.distanceLeft =
            marqueed_el.totalDistance -
            (marqueed_el.containerWidth - plugin.getPosition(elName));
        } else {
